% playresponsetone.m
function playresponsetone(p,response)
% response: 2 - correct, 3 - incorrect, 4 - wrong side tone

signal = .5;
freq = p.sounds(response).toneFreq;
errorfeedbacktime = p.sounds(response).toneDur;

Snd('Play',MakeBeep(freq,errorfeedbacktime));
